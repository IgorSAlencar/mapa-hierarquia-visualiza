...
function fmtProducaoCompacta(reais: number): string {
  const mil = reais / 1000;
  return `${mil.toLocaleString('pt-BR', { maximumFractionDigits: 1, minimumFractionDigits: 1 })} mil`;
}
    ...
    <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
      ...
      <div className="grid grid-cols-3 divide-x divide-slate-100 border-b border-slate-100 px-1 py-3">
        <div className="px-2 text-center">
          <p className="text-[10px] font-semibold uppercase tracking-wide text-slate-500">Produção (R$)</p>
          <p className="mt-1 text-lg font-bold tabular-nums leading-none text-slate-900">
            {p.producaoMes > 0 ? fmtProducaoCompacta(p.producaoMes) : '—'}
          </p>
      ...
      <div className="border-b border-slate-100 px-3 py-3">
        <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
          <p className="text-xs font-semibold text-slate-800">Evolução da produção (R$ mil)</p>
      ...
      <div className="grid grid-cols-1 gap-2 border-t border-slate-100 px-3 py-3 sm:grid-cols-2">
        <button
          type="button"
          onClick={onVerLojasQueda}
          className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-300 bg-white py-2.5 text-xs font-semibold text-slate-800 shadow-sm transition-colors hover:bg-slate-50"
        >
        ...
        <button
          type="button"
          onClick={() => setDetalhesAberto((o) => !o)}
          className="inline-flex items-center justify-center gap-2 rounded-xl bg-sky-600 py-2.5 text-xs font-semibold text-white shadow-sm transition-colors hover:bg-sky-700"
        >
      ...
      {p.subprodutos.length > 0 && detalhesAberto && (
                ...
                {s.valorLegenda ? (
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="text-right">
                      ...
                      <p className="mt-0.5 font-semibold tabular-nums text-slate-900">{fmtMoney(s.producaoMes)}</p>
                    </div>
                  </div>
                ) : (
                  <div className="flex flex-col gap-0.5 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <span className="flex flex-wrap items-center gap-x-3 gap-y-1 text-slate-600">
                      ...
                      <span className="tabular-nums font-semibold text-slate-900">{fmtMoney(s.producaoMes)}</span>
                    </span>
                  </div>
                )}
              </li>
